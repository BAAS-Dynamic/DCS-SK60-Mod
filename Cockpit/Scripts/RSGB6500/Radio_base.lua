...
erpm_base_clip.name 			    = "eRadio_base_clip"
erpm_base_clip.primitivetype   	    = "triangles"
erpm_base_clip.vertices 		    = { {1, aspect}, { 1,-aspect}, { -1,-aspect}, {-1,aspect},} --四个边角
...
erpm_base_clip.element_params       = {"RADIO_DSP_ENABLE"}              -- Initialize the main display control
...
RTOP_text_box.element_params    = {"RADIO_DSP_UPPER_TEXT","RADIO_DSP_ENABLE"} -- top left first line display
...
RTOP_text_box.parent_element    = "eRadio_base_clip"  --Parent node name - can bind parent nodes that are not on the same layer
Add(RTOP_text_box)
...
RTOP_text_box.stringdefs        = {0.9*0.0095,0.9 * 0.0095, 0, 0}    --{ecrase vertical si inf a 0.01,ecrase lateral * streccth, 0, 0} The first value controls the width, the second value controls the height
RTOP_text_box.formats           = {"%s","%s"} -- The output is set here, similar to the printf model of C.% is the output type at the beginning, and the following %s is the input type
RTOP_text_box.element_params    = {"RADIO_DSP_LOWER_TEXT","RADIO_DSP_ENABLE"} -- top left first line display
RTOP_text_box.controllers       = {{"text_using_parameter",0},{"opacity_using_parameter",1}}
RTOP_text_box.collimated        = true
RTOP_text_box.use_mipfilter     = true
RTOP_text_box.additive_alpha    = true
